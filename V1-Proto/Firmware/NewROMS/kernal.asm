...
;8563 VDC (SteveLCD-specific)
VDC_STATUS    := $F900 ;Read
VDC_ADDR      := $F900 ;Write
VDC_DATA      := $F901
...
;       jsr     LB688_GET_KEY_NONBLOCKING
        jmp patch_LB6DF_GET_KEY_BLOCKING
...
;       ldx     #MON_CMD_COUNT-1 ;X = point to last entry in commands table
        ldx     #patch_MON_CMD_COUNT-1
LC78A_CMD_SEARCH_LOOP:
;PATCH:
;       cmp     MON_COMMANDS,x
        cmp patch_MON_COMMANDS,x
        beq     LC794_FOUND_CMD
        dex
...
;       jmp     MON_CMD_LOAD_SAVE_VERIFY
        jmp patch_MON_CMD_LOAD_SAVE_VERIFY
; ----------------------------------------------------------------------------
MON_CMD_MEMORY:
        bcs     LC7B9
        jsr     T0TOT2
...
; ----------------------------------------------------------------------------
; Start of area available for new code
;
; This area ($D480-F6FF) is available for new code.  It originally contained
; a part of the C128 BASIC ROM, which was filler not used by the CLCD.
; ----------------------------------------------------------------------------

    .assert * = $d480, error

;Hardware reset vector points here
patch_RESET:
    sei
    cld
    ldx #$ff
    txs
    ;Init VDC as early as possible or else system crashes mysteriously
    jsr vdc_init
    jsr via2_init
    jsr acia_init
    ;Send the 80x16 text screen out Centronics
    jsr dump_screen_text_to_centronics
    ;Continue with the real reset
    jmp RESET

;Patch called from LCDsetupSet so that the 8563 VDC is
;initialized whenever the LCD controller is initialized.
patch_LCDsetupSet:
    pha
    phx
    jsr vdc_init
    plx
    pla
    sta SETUP_LCD_A       ;do the instruction we patched over to get here
    rts                   ;return to the middle of LCDsetupSet

;Patch called from KR_ShowChar_ so that anything printed to
;the screen via the KERNAL will also be sent to Centronics (raw)
;and ACIA (translated to ASCII).
patch_KR_ShowChar_:
    pha
    phx
    phy
    ...
    beq @crlf ;CR->CRLF
    pha
    jsr centronics_chrout
    pla
    ldx #1 ;translation mode
    jsr TRANSL_OUTGOING_CHAR
    jsr acia_write
    bra @done
@crlf:
    jsr centronics_chrout
...
@done:
    ...
    plx
    pla
    bit $0384 ;do the instruction we patched over to get here
    rts

;Patch called from LB6DF_GET_KEY_BLOCKING so that reading a key reads
;from the ACIA instead of the keyboard.  Characters received are translated
;from ASCII to PETSCII.
patch_LB6DF_GET_KEY_BLOCKING:
    phx
    phy
    jsr acia_read   ;blocks until a char is received
    pha
    jsr acia_write  ;echo char back
    pla
    cmp #$08
    beq @backspace
    cmp #$1b
...
@escape:
    ...
    ;right 1B 5B 43
    ;left  1B 5B 44
    jsr acia_read   ;read the 5B
    jsr acia_read   ;read the 41/42/43/44
    sec
    sbc #$41
    tax
    lda @petscii_cursors,x
@done:
    ply
    plx
    rts

;Monitor command table has been patched to add commands.
patch_MON_COMMANDS:
    .byte   "X" ;Exit
    .byte   "M" ;Memory
    .byte   "R" ;Registers
    ...
    .byte   ";" ;Modify Registers
    .byte   "W" ;Walk
patch_MON_CMD_LOAD_IDX = * - patch_MON_COMMANDS
    .byte   "L" ;Load
    .byte   "S" ;Save
    .byte   "V" ;Verify
    ;New commands. Commands can only be added here, not above.
    .byte   "@" ;DOS Wedge
patch_MON_CMD_COUNT = * - patch_MON_COMMANDS

;Patch jumped to by monitor when it gets a command L,S,V
;or any new command in the table above.  The command is
;in V1541_FNLEN on entry.
patch_MON_CMD_LOAD_SAVE_VERIFY:
    lda V1541_FNLEN
    cmp #'@'
    bne @original_lsv
    ;Got "@" wedge command
    jsr PRIMM
    .byte "wedge goes here",0
    jmp MON_MAIN_INPUT
@original_lsv:
    jmp MON_CMD_LOAD_SAVE_VERIFY


;Init VIA2 so Centronics port works
via2_init:
    stz     VIA2_PORTA
    lda     #$FF
    sta     VIA2_DDRA
...
;Initialize VDC by writing all registers in the table
vdc_init:
    ldx #0
@loop:
    lda @table,x	        ;get 8563 register #
    bmi @done             ;...branch if end-of-table
    sta VDC_ADDR
@wait_ready:      ;wait for vdc to be ready
    bit VDC_STATUS
    bpl @wait_ready
    lda @table+1,x        ;get data for this register
    sta VDC_DATA          ;write it to vdc
    inx                   ;advance to next pair in table
    inx
    bra @loop
@done:
    rts
@table:
    .byte 0,$7e	  ;H Total (Clock cycles per rasterline - 1)
    .byte 1,80	  ;H Displayed	= 80 col
    .byte 2,$66	  ;H Sync
    .byte 3,$49	  ;Sync width
    .byte 4,$20	  ;V Total
    .byte 5,$00	  ;V Total Adjust
    .byte 6,16	  ;V Displayed	= 16 lines
    .byte 7,$1d	  ;V Sync Position
    ;8563 NTSC
    .byte 8,$00	  ;Mode (normal interlace etc)
    .byte 9,$07	  ;Character height	= 8 pixels (height-1)
    .byte 10,$20	;Cursor Start
    .byte 11,$07	;Cursor End
    .byte 12,>$0800	;Display Start HI ****
    .byte 13,<$0800	;Display Start LO
    .byte 14,0	  ;Cursor Address HI
    .byte 15,0 	  ;Cursor Address LO
    .byte 20,0	  ;Attribute Address HI	**** Can be zero
    .byte 21,0	  ;Attribute Address LO
    .byte 23,$08	;Character Height Visible = 8 pixels
    .byte 24,$20	;RVS, Blink enable
    .byte 25,$00+7 ;Attributes=off, H.Scroll=%111 Note: 8563 Rev 8+(?) / 8568(?) specific!	****
    .byte 26,$f0	;FG/BG colour
    .byte 27,48	  ;Address increment	**** 80+48=128
    .byte 28,$80+$10 ;Font Address=$8000, RAM Type=64K	****
    .byte 29,$07	;Underline raster
    .byte 34,$7d	;Display Enable begin
    .byte 35,$64	;Display Enable end
    .byte 36,$05	;DRAM Refresh		**** May need changing later
    .byte 22,$78	;Character Width/Total = 8/8 (width-1)
    .byte $ff	    ;END OF TABLE marker

dump_screen_text_to_centronics:
    lda #<$0800
    sta FNADR
    lda #>$0800
    sta FNADR+1

    lda #16-1 ;16 lines to print
    sta SXREG
...
;Send a byte to the Centronics port
;VIA2 must be initialized first
;Does not check /BUSY
centronics_chrout:
    ldx     #$03
@loop:
    dex
    bpl @loop ;delay a bit
...
acia_init:
    sta ACIA_ST     ;Programmed Reset
    LDA #$0B
    sta ACIA_CMD    ;No interrupts, N81
    lda #%00011110  ;1 stop bit, 8 data bits, baud rate generator, 9600 bps
    sta ACIA_CTRL
    rts
...
; ----------------------------------------------------------------------------
; End of area available for new code
; ----------------------------------------------------------------------------

        .assert * < $f700, error

        ;Fill the unused space so the KERNAL resumes at $F700
        fillsize = $f700 - *
        .res fillsize, $ff
